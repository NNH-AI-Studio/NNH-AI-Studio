...
CREATE POLICY "Users can update own profile"
  ...
  TO authenticated
  USING (auth.uid() = id); -- 💡 تم إزالة WITH CHECK المكررة
...
CREATE TABLE IF NOT EXISTS gmb_accounts (
  ...
  -- 🛡️ إصلاح الأمان/النزاهة: منع المستخدم من ربط نفس حساب GMB أكثر من مرة
  CONSTRAINT unique_account_per_user UNIQUE (user_id, account_id)
);
...
CREATE POLICY "Users can update own GMB accounts"
  ...
  TO authenticated
  USING (auth.uid() = user_id); -- 💡 تم إزالة WITH CHECK المكررة
